...
position and use it even when point has moved elsewhere."
  (chai-test--with-temp-org "[[chai:important][target]] other text"
    ...
    (let* ((menu (make-sparse-keymap))
           ...
           (cmd (lookup-key result-menu [chai chai-copy-text])))
      (should (commandp cmd))
      (funcall cmd)
      (should (string= (current-kill 0 t) "target")))))
...
(ert-deftest chai-test-context-menu-shows-chai-action-on-plain-click ()
  ...
  (chai-test--with-temp-org "plain text"
    (let* ((menu (make-sparse-keymap))
           (click-event (list 'mouse-3 (posn-at-point 1 (selected-window))))
           (result-menu (chai-context-menu menu click-event)))
      (should (commandp (lookup-key result-menu [chai chai-add-comment])))
      (should (commandp (lookup-key result-menu [chai chai-highlight-type-important])))
      (should (commandp (lookup-key result-menu [chai chai-highlight-region])))
      (should (commandp (lookup-key result-menu [chai chai-highlight-annotate]))))))
  ...
  (chai-test--with-temp-org "select this text"
    (let ((chai-highlight-types '(("custom" . chai-highlight-important)
      ...
      (let* ((menu (make-sparse-keymap))
             (click-event (list 'mouse-3 (posn-at-point 1 (selected-window))))
             (result-menu (chai-context-menu menu click-event))
             (cmd (lookup-key result-menu [chai chai-highlight-type-custom])))
        (should (commandp cmd))
        (should (commandp (lookup-key result-menu [chai chai-highlight-type-review])))
        (should-not (lookup-key result-menu [chai chai-highlight-type-important]))
        (deactivate-mark)
        (funcall cmd)
        (should (string= (buffer-string)
...
(ert-deftest chai-test-context-menu-region-highlight-uses-captured-region ()
  ...
  (chai-test--with-temp-org "select this text"
    ...
    (let* ((menu (make-sparse-keymap))
           (click-event (list 'mouse-3 (posn-at-point 1 (selected-window))))
           (result-menu (chai-context-menu menu click-event))
           (cmd (lookup-key result-menu [chai chai-highlight-type-important])))
      (deactivate-mark)
      (goto-char (point-max))
      (should (commandp cmd))
      (funcall cmd)
  ...
  (chai-test--with-temp-org "select this text"
    ...
    (cl-letf (((symbol-function 'completing-read) (lambda (&rest _) "idea"))
      ...
      (let* ((menu (make-sparse-keymap))
             (click-event (list 'mouse-3 (posn-at-point 1 (selected-window))))
             (result-menu (chai-context-menu menu click-event))
             (cmd (lookup-key result-menu [chai chai-highlight-annotate])))
        (deactivate-mark)
        (goto-char (point-max))
        (should (commandp cmd))
        (funcall cmd)
...
(ert-deftest chai-test-context-menu-add-comment-uses-captured-region ()
  ...
  (chai-test--with-temp-org "my note"
    ...
    (let* ((menu (make-sparse-keymap))
           (click-event (list 'mouse-3 (posn-at-point 1 (selected-window))))
           (result-menu (chai-context-menu menu click-event))
           (cmd (lookup-key result-menu [chai chai-add-comment])))
      (deactivate-mark)
      (should (commandp cmd))
      (funcall cmd)
      (should (string= (buffer-string)
...
(ert-deftest chai-test-context-menu-groups-actions-under-one-entry ()
  "All Chai actions live in a single Chai submenu at the top level."
  (chai-test--with-temp-org "[[chai:important][target]] other text"
    (let* ((chai-stored-notes '((:title "Book")))
           (menu (make-sparse-keymap))
           (click-event (list 'mouse-3 (posn-at-point 1 (selected-window))))
           (result-menu (chai-context-menu menu click-event))
           (top (delq nil (mapcar (lambda (binding)
                                    (and (consp binding) (car binding)))
                                  (cdr result-menu)))))
      (should (equal (sort top #'string<) '(chai chai-separator)))
      (dolist (key '(chai-change-type chai-highlight-region chai-add-comment
                     chai-store-notes chai-insert-stored-notes))
        (should (commandp (lookup-key result-menu (vector 'chai key))))))))
